"""Test module for monitoring app."""
...
class DeviceModelTests(TestCase):
    ...
    def setUp(self):
        """Set up a test user for device tests."""
...
class ReadingModelTests(TestCase):
    ...
    def setUp(self):
        """Set up a test user and device for reading tests."""
        self.user = User.objects.create_user(
            username="testuser", email="test@example.com", password="testpass123"
        )
    ...
    def test_reading_str_method(self):
        ...
        reading = Reading.objects.create(device=self.device, ph=7.0, tds=150.0, ntu=2.5)
        expected = f"Reading {reading.pk} - {reading.timestamp}"
...
class APIViewTests(TestCase):
    ...
    def setUp(self):
        """Set up a test user and device for API tests."""
        self.user = User.objects.create_user(
            username="testuser", email="test@example.com", password="testpass123"
        )
